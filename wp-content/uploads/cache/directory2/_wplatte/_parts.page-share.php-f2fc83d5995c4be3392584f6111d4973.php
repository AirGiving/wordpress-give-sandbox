<?php //netteCache[01]000594a:2:{s:4:"time";s:21:"0.64087800 1608561953";s:9:"callbacks";a:4:{i:0;a:3:{i:0;a:2:{i:0;s:6:"NCache";i:1;s:9:"checkFile";}i:1;s:105:"/home/customer/www/evernewjia.com/public_html/airgiving/wp-content/themes/directory2/parts/page-share.php";i:2;i:1607214400;}i:1;a:3:{i:0;a:2:{i:0;s:6:"NCache";i:1;s:10:"checkConst";}i:1;s:20:"NFramework::REVISION";i:2;s:22:"released on 2014-08-28";}i:2;a:3:{i:0;a:2:{i:0;s:6:"NCache";i:1;s:10:"checkConst";}i:1;s:15:"WPLATTE_VERSION";i:2;s:5:"2.9.2";}i:3;a:3:{i:0;a:2:{i:0;s:6:"NCache";i:1;s:10:"checkConst";}i:1;s:17:"AIT_THEME_VERSION";i:2;s:6:"4.0.19";}}}?><?php

// source file: /home/customer/www/evernewjia.com/public_html/airgiving/wp-content/themes/directory2/parts/page-share.php

?><?php
// prolog NCoreMacros
list($_l, $_g) = NCoreMacros::initRuntime($template, 'a7ed7l4qxx')
;
// prolog NUIMacros
...
<?php endforeach; } elseif ($wp->isAuthor) { $qobj = get_queried_object() ;if ($qobj and isset($qobj->ID)) { $shareUrl 				= get_author_posts_url( $qobj->ID ) ;$shareTitle 			= get_the_author_meta( 'display_name', $qobj->ID ) ;$shareDescription		= get_the_author_meta( 'description', $qobj->ID ) ;} } elseif ($wp->isCategory or $wp->isTax('ait-events-pro') or $wp->isTax('ait-items') or $wp->isTax('ait-locations') or $wp->isTax('events')) { $qobj = get_queried_object() ;if ($qobj and isset($qobj->term_id)) { $shareUrl 				= get_category_link(intval($qobj->term_id)) ;$shareTitle 			= $qobj->name ;$shareDescription		= $qobj->description ;} } elseif ($wp->isPostTypeArchive('ait-special-offer')) { $qobj = get_queried_object() ;if ($qobj and isset($qobj)) { $shareUrl 				= get_post_type_archive_link('ait-special-offer') ?>
		<?php ob_start() ;echo $template->printf(__('Archives: %s', 'wplatte'), $archive->title) ;$titleOfArchive = ob_get_clean() ?>

<?php $shareTitle 			= $titleOfArchive ;} } elseif ($wp->isPostTypeArchive('ait-item')) { $qobj = get_queried_object() ;if ($qobj and isset($qobj)) { $shareUrl 				= get_post_type_archive_link('ait-item') ?>
		<?php ob_start() ;echo $template->printf(__('Archives: %s', 'wplatte'), $archive->title) ;$titleOfArchive = ob_get_clean() ?>

<?php $shareTitle 			= $titleOfArchive ;} } elseif ($wp->isPostTypeArchive('ait-event-pro')) { $qobj = get_queried_object() ;if ($qobj and isset($qobj)) { $shareUrl 				= get_post_type_archive_link('ait-event-pro') ?>
		<?php ob_start() ;echo $template->printf(__('Archives: %s', 'wplatte'), $archive->title) ;$titleOfArchive = ob_get_clean() ?>

<?php $shareTitle 			= $titleOfArchive ;} } else { if (AitWoocommerce::enabled()) { if (AitWoocommerce::currentPageIs('shop')) { $shareUrl 				= get_permalink( wc_get_page_id( 'shop' ) ) ?>
