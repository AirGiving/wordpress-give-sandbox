...
class Listify_WP_Job_Manager extends listify_Integration {
	...
	public function job_manager_output_jobs_defaults( $default ) {
		...
		if ( isset( $_GET['search_categories'] ) ) {
			$search_categories = is_array($_GET['search_categories']) ? $_GET['search_categories'] : array($_GET['search_categories']);
			$categories = array_filter( array_map( 'esc_attr', $search_categories  ), 'listify_array_filter_deep' );
