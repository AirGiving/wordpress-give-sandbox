...
{* ********************************************************* *}
	...
	{var $pageShare = $el->option(pageShare) && !$wp->isSearch}
	{var $showPager = ''}
	{var $titleExcerpt = ''}
	{var $categoryColor = ''}
...
{loop as $post}
	...
	{* ITEM ICON ****** *} {if $wp->isSingular(job-offer)}
								{if $post->imageUrl('thumbnail')}
									{var $categoryIcon = $post->imageUrl('thumbnail')}
								{/if}
							{/if}
		...
		{/if}
			...
			<div class="entry-title {$titleClass}">
				...
				<div class="entry-title-wrap">
					...
					{if isset($category) and isset($categoryIcon)}
						<div class="cat-icon"><span {if !empty($categoryColor)}style="background: {!$categoryColor};"{/if}><img src="{$categoryIcon}" alt="{!$category->name}"></span></div>
